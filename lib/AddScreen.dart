...
class _AddScreenState extends State<AddScreen> {
        ...
        child: Column(
          children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _timePicker("From", _startTime, (newTime) => setState(() => _startTime = newTime)),
                _timePicker("To", _endTime, (newTime) => setState(() => _endTime = newTime)),
              ],
            ),
            const SizedBox(height: 30),
            // Card for Title
            ...
            SizedBox(height: 20),
            Row(
              mainAxisAlignment: MainAxisAlignment.start, // Aligns children to the start (left side)
              children: [
                Text(
                  "Choose Priority",
                  style: TextStyle(
            ...
            SizedBox(height: 20),
            Row(
  mainAxisAlignment: MainAxisAlignment.center, // Aligns children to the start (left side)
  children: [
    _priorityButton("High", Colors.red),
    SizedBox(width: 50), // Optional: Add space between buttons
    _priorityButton("Medium", Colors.grey),
    SizedBox(width: 50), // Optional: Add space between buttons
    _priorityButton("Low", Colors.lightGreen),
  ],
),
  ...
  Widget _timePicker(String label, TimeOfDay initialTime, Function(TimeOfDay) onTimeSelected) {
    return Column(
      children: [
        Text(
        label,
        style: TextStyle(
          fontWeight: FontWeight.bold,
          fontSize: 18, // Increase label font size here
        ),
      ),
        GestureDetector(
          onTap: () async {
  ...
  return Container(
    width: 120, // Set the desired height here
    child: ElevatedButton(
      style: ElevatedButton.styleFrom(
